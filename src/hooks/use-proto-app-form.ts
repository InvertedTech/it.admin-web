      ...
      onSubmitAsync: async ({ value, formApi }) => {
        ...
        if (res.kind === "invalid") {
          ...
          const filteredViolations = rawViolations.filter((v: any) => {
            const parts = Array.isArray(v?.field)
              ? (v.field as any[]).map((p: any) => p?.name ?? p?.jsonName ?? p?.localName).filter(Boolean)
              : (((v as any)?.fieldPath?.elements ?? (v as any)?.field?.elements)?.map((e: any) => e?.name) ?? []);
            const path = Array.isArray(parts) && parts.length ? parts.join(".") : undefined;
            const msg = (v?.message ?? "") as string;
            const ruleId = (v?.ruleId ?? "") as string;
            const isUrlUri = path?.toLowerCase() === "public.url" && (/uri/i.test(ruleId) || /uri/i.test(msg));
            return !isUrlUri;
          });
          ...
          try {
            // eslint-disable-next-line no-console
            console.log('[useProtoAppForm] mapped submit errors (after filter)', result);
          } catch {}
        }
        if (res.kind === "error") {
        ...
        try {
          const fields: Record<string, string | string[]> =
            ((result as any)?.fields ? { ...(result as any).fields } : {}) as any;
          ...
          if (cd?.case === 'Video') {
            ...
            if (!rum && !yt) {
              fields[base + 'RumbleVideoId'] = 'Provide a Rumble or Youtube Id';
              fields[base + 'YoutubeVideoId'] = 'Provide a Youtube or Rumble Id';
            }
            if (!String(cd?.value?.HtmlBody ?? '').trim()) {
              fields[base + 'HtmlBody'] = 'Body is required';
            }
          } else if (cd?.case === 'Written' || cd?.case === 'Audio' || cd?.case === 'Picture') {
            if (!String(cd?.value?.HtmlBody ?? '').trim()) {
              fields[base + 'HtmlBody'] = 'Body is required';
            }
          }
