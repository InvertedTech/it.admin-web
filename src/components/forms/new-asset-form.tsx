...
import {
  AudioAssetDataSchema,
} from '@inverted-tech/fragments/Content/AudioAssetRecord_pb';
import {
  ImageAssetDataSchema,
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex gap-2">
          {fileName && onClear ? (
            <Button type="button" variant="outline" onClick={onClear}>Remove</Button>
          ) : null}
...
function AutoSlugger({ form, titlePath, urlPath }: { form: any; titlePath: string; urlPath: string }) {
  return (
...
export function NewImageAssetForm() {
  const form = useProtoAppForm({
    ...
    defaultInit: {
      Public: { Title: '', Caption: '', URL: '', MimeType: '', Height: 0, Width: 0, Data: new Uint8Array() },
      Private: { OldAssetID: '' },
    },
    onSubmitAsync: async ({ value }) => {
      // No action yet — just log payload
      console.log('Create Image Asset payload', value);
    },
  });
  ...
  async function onFileSelected(file: File) {
    ...
    try {
      ...
      const img = new Image();
      img.onload = () => {
        form.setFieldValue('Public.Width', (img as any).naturalWidth || img.width || 0);
        form.setFieldValue('Public.Height', (img as any).naturalHeight || img.height || 0);
  ...
  return (
    <FormCard cardTitle="Create Image Asset" cardDescription="Upload and describe an image.">
      <form
        onSubmit={(e) => {
          e.preventDefault();
          form.handleSubmit();
      ...
      >
        ...
        <AutoSlugger form={form} titlePath="Public.Title" urlPath="Public.URL" />
          ...
          {previewUrl && (
            <div className="md:col-span-2">
              <div className="rounded-md border p-3">
                ...
                <img src={previewUrl} alt="Selected image preview" className="max-h-64 w-auto rounded" />
              </div>
            </div>
          )}

          <form.AppField name="Public.Title" children={(field) => <field.TextField label="Title" />} />
          <form.AppField name="Public.URL" children={(field) => <field.TextField label="URL" disabled />} />
          <div className="md:col-span-2">
            <form.AppField name="Public.Caption" children={(field) => <field.TextField label="Caption" />} />
          </div>
              ...
              <form.Subscribe selector={(s: any) => !!s?.isSubmitting}>
                {(isSubmitting: boolean) => (
                  <Button type="submit" disabled={isSubmitting}>
                    {isSubmitting ? (<><Spinner className="mr-2" /> Validating...</>) : 'Validate'}
                  </Button>
                )}
              </form.Subscribe>
            }
...
export function NewAudioAssetForm() {
  const form = useProtoAppForm({
    ...
    defaultInit: {
      Public: { Title: '', Caption: '', URL: '', MimeType: '', LengthSeconds: 0, Data: new Uint8Array() },
      Private: { OldAssetID: '' },
    },
    onSubmitAsync: async ({ value }) => {
      console.log('Create Audio Asset payload', value);
    },
  });
  ...
  async function onFileSelected(file: File) {
    ...
    try {
      ...
      audio.addEventListener('loadedmetadata', () => {
        ...
        if (Number.isFinite(dur)) form.setFieldValue('Public.LengthSeconds', dur);
      });
    } catch {}
  }

  return (
    <FormCard cardTitle="Create Audio Asset" cardDescription="Upload and describe an audio file.">
      <form
        onSubmit={(e) => {
          e.preventDefault();
          form.handleSubmit();
