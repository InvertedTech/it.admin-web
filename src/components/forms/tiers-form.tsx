...
export type TierConfig = { Tiers: Tier[] };
					...
					{(field: any) =>
						field.state.value.length ? (
							...
							<div className="rounded-md border p-6 text-sm text-muted-foreground">
								No tiers yet. Click <b>Add Tier</b> to get started.
							</div>
						)
					}
				</form.Field>
...
function TierItem({
	...
	const name = String(form.getFieldValue(namePath) ?? 'Untitled');
	const color = String(form.getFieldValue(colorPath) ?? '#7c3aed');
	...
	return (
		...
		>
			{/* Header – full clickable trigger, single chevron (from shadcn) */}
				...
				<div className="flex items-center gap-3">
					...
					<span className="font-medium">
						...
						<span className="font-normal text-muted-foreground">
							· {Number.isFinite(amount) ? centsToCurrency(amount) : '—'}
						</span>
					</span>
				</div>
			</AccordionTrigger>
			...
			<AccordionContent className="px-3 pb-4">
				...
				<div className="grid grid-cols-1 gap-4 md:grid-cols-3">
					...
					<form.Field name={amountPath}>
						{(field: any) => (
							<div className="grid gap-2">
								<Label htmlFor={`${amountPath}`}>Amount (cents)</Label>
