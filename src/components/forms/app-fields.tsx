...
import { ChevronsUpDownIcon, CheckIcon, XIcon, CalendarIcon } from 'lucide-react';
...
export function DateTimeField({
  ...
  const [text, setText] = useState<string>(() => {
    const d = initial;
    return d ? d.toLocaleDateString('en-US', { day: '2-digit', month: 'long', year: 'numeric' }) : '';
  });
  ...
  return (
    <form.Subscribe selector={(s: any) => ({ submit: s?.submitErrors, sync: s?.errors })}>
      {(errState: any) => {
        const submitField = matchFieldErrors(errState?.submit?.fields as any, field.name) ?? [];
        const syncField = matchFieldErrors(errState?.sync?.fields as any, field.name) ?? [];
        const combined = [
          ...(Array.isArray(field.state.meta.errors) ? (field.state.meta.errors as any) : []),
          ...submitField,
          ...syncField,
        ];
        const errors = normalizeFieldErrors(combined as any) ?? [];
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <Button type="button" variant="outline" size="sm" onClick={() => {
                  const d = new Date();
                  setDate(d); setMonth(d); setText(fmtDate(d)); commit(d);
                }}>Now</Button>
                <Button type="button" variant="outline" size="sm" onClick={() => {
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => {
                  ...
                  setDate(d); setMonth(d); setText(fmtDate(d)); commit(d);
                }}>Next Mon 9:00</Button>
                <Button type="button" variant="ghost" size="sm" onClick={() => {
                  setDate(undefined); setText(''); field.handleChange(undefined);
                }}>Clear</Button>
              </div>
            </div>
            {isInvalid && <FieldError errors={errors} />}
